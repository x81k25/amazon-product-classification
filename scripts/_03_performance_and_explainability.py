...
def main(data_dir):
	print("Starting model explanation process...")
	...
	model_files = glob.glob(
		os.path.join(data_dir, 'model_artifacts', 'xgb_best_model_*.pkl'))
	...
	predictions_df = pd.read_parquet(
		os.path.join(data_dir, '02_predictions.parquet'))
	...
	X_test = pd.read_parquet(os.path.join(data_dir, '02_x_test.parquet'))
	...
	cm_df = pd.DataFrame(
		cm,
		index=[reverse_mapping.get(idx, f"Unknown-{idx}") for idx in
			   class_indices],
	...
	overall_metrics.to_parquet(
		os.path.join(data_dir, '03_overall_metrics.parquet'))
	per_class_df.to_parquet(
		os.path.join(data_dir, '03_per_class_metrics.parquet'))
	cm_df.to_parquet(os.path.join(data_dir, '03_confusion_matrix.parquet'))
	...
	for class_idx in range(len(category_mapping)):
		if isinstance(shap_values, list):  # Multi-output model
			if class_idx < len(shap_values):
				feature_importance[class_idx] = np.abs(
					shap_values[class_idx]).mean(
					0)
	...
	global_importance = feature_importance_df.groupby('feature_name')[
		'importance'].apply(
		lambda x: np.mean(
			[i if isinstance(i, (int, float)) else i.mean() for i in x])
	).reset_index()

	global_importance = global_importance.sort_values('importance',
													  ascending=False)

	global_importance.to_parquet(
		os.path.join(data_dir, '03_global_importance.parquet'))

	print("Model explanation process completed successfully")


if __name__ == "__main__":
	parser = argparse.ArgumentParser(
		description="Generate model explanations and performance metrics")
...
# -----------------------------------------------------------------------------
# end of _03_model_explanation.py
# -----------------------------------------------------------------------------