...
def main(input_dir):
	...
	print("Applying sparse PCA to details fields...")
	...
	print(
		"Generating BERT embeddings for title field (this may take a while)...")
	...
	# repeat for features
	print(
		"Generating BERT embeddings for features field (this may take a while)...")
	features_embeddings = embed_dataframe_column(df, "features_concat")

	print("Reducing features embeddings dimensionality...")
	reduced_features_emb = svd.fit_transform(features_embeddings)

	for i in range(len(reduced_features_emb[0])):
		df = df.with_columns(
			pl.Series(f"features_emb_{i}",
					  [emb[i] for emb in reduced_features_emb])
		)

	# repeat for description
	print(
		"Generating BERT embeddings for description field (this may take a while)...")
	description_embeddings = embed_dataframe_column(df, "description_concat")

	print("Reducing description embeddings dimensionality...")
	reduced_description_emb = svd.fit_transform(description_embeddings)

	for i in range(len(reduced_description_emb[0])):
		df = df.with_columns(
			pl.Series(f"desc_emb_{i}",
					  [emb[i] for emb in reduced_description_emb])
		)
	...
	df.write_parquet(os.path.join(data_dir, "01_products_engineered.parquet"))
